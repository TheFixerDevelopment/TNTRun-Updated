...
class GameHandler{
    ...
    public function startArenaCountDown(){
        ...
        $this->countDownTaskId = $this->tntRun->getScheduler()->scheduleDelayedRepeatingTask(new CountDownTask($this->tntRun, $this->arena), 20, 20)->getTaskId();
    ...
    public function stopArenaCountDown(){
        $this->tntRun->getScheduler()->cancelTask($this->countDownTaskId);
    ...
    public function startArena(){
        ...
        $this->gameTaskId = $this->tntRun->getScheduler()->scheduleDelayedRepeatingTask(new GameTask($this->tntRun, $this->arena), 1200, 1200)->getTaskId();
    }
    ...
    public function startArenaRegen(){
        ...
        foreach($this->arena->getStructureManager()->getFloors() as $floorY){
            $this->tntRun->getScheduler()->scheduleDelayedTask(new AddfloorTask(
                $this->tntRun,
